...
// Runs `gleam update` for all modules or one.
// Usage: bun scripts/update.js [module-name]
...
function updateModule(name) {
  ...
  const result = spawnSync(["gleam", "update"], {
    cwd,
    stdout: "inherit",
    stderr: "inherit",
  });
  if (result.exitCode !== 0) {
    console.error(`  ✗ ${name} dependency update failed`);
    return false;
  }
  return true;
}
...
for (const name of modules) {
  if (!updateModule(name)) allPassed = false;
}
