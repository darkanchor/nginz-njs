...
Typed `ngx.fetch()` wrapper. Phases 1–5 are complete: an expanded pure request model, a typed fetch execution path with body and timeout support, rich `ClientError` variants, response classification helpers, retry/timeout policy wrappers, composable middleware, and response body helpers.
...
- Timeout is enforced via `ngx.fetch()` options when `timeout_ms` is set on the `Request`
...
- `Timeout`, `InvalidUrl`, and `InvalidRequest` error variants are defined for consumers to pattern-match but `execute()` currently only produces `FetchFailed`
